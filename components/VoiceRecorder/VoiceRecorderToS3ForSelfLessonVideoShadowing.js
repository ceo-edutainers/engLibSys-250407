...
export default class VoiceRecorderToS3ForSelfLessonVideoShadowing extends React.Component {
  ...
  handleCalAudioDurationFromDB(audioDurationFromDB, YourFileDuration, Wari) {
    ...
    var audioMin = parseInt(audioDurationFromDB.split(':')[0]) //ビデオの分を取り出す 3:18の場合、3
    var audioSec = parseInt(audioDurationFromDB.split(':')[1]) //ビデオの秒を取り出す 3:18の場合、18

    var totalMinToSec = audioMin * 60 //全ての分を秒で直す 3分の場合 3*6-=180秒

    if (
      this.props.shadowingHWAmount == 'first half' ||
      this.props.shadowingHWAmount == 'second half'
    ) {
      var totalSec = parseInt(parseInt(totalMinToSec) + parseInt(audioSec)) / 2
    } else {
      var totalSec = parseInt(totalMinToSec) + parseInt(audioSec)
    }
    console.log('test-audioMin', audioMin)
    console.log('test-audioSec', audioSec)
    console.log('test-totalMinToSec', totalMinToSec)
    console.log('test-totalSec', totalSec)
    //DBのAudioのWari(例:0.7 7/10の長さになる)の長さを知る: 生徒が録音したファイルの長さが、元々のDB Audio Fileの7/10以上の長さにならないとエラーを足すために
    var totalSecWari = parseInt(totalSec * Wari)

    console.log('test-totalSecWari:', totalSecWari)
    console.log('test-YourFileDuration', YourFileDuration) //生徒が録音した秒

    if (totalSecWari > YourFileDuration) {
      // alert('録音した長さが短いです。決まった分量をきちんと録音してください。')
  ...
  stop = () => {
    const fetchAudioData = async () => {
      try {
        ...
        //長さをチェックする時に以下のコードを使用する。現在は使用しない
