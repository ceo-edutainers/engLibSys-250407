...
const Step3OST = () => {
  ...
  const [firstOrder, setFirstOrder] = useState(
    '①&nbsp;前のステップのアウトラインをもとに、描写を加えたり、より詳細な説明を加えることで、指定された単語数に近い文章を作成します。'
  )
  const [secondOrder, setSecondOrder] = useState(
    '②&nbsp;Title・Introduction・Body・Conclusionに分けて書きます。'
  )
  const [thirdOrder, setThirdOrder] = useState(
    '③&nbsp;このスクリプトを元に25分〜50分レッスンを受けますので、できるだけ沢山書いてください。'
  )
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.data.length > 0) {
          ...
          if (
            response.data[0].script ==
            'Topic:\n\nIntroduction:\n\nBody:\n\nConclusion:'
          ) {
            setWholeScript('Topic:\n\nIntroduction:\n\nBody:\n\nConclusion:')
          } else {
