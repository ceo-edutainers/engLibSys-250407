    ...
    if (localStorage.getItem('loginStatus') == 'true') {
      ...
      const fetchData2 = async () => {
        try {
          ...
          setMonsterCount(response.data.length)
        } catch (error) {
          console.log(error)
        }
      }
  ...
  return (
    <>
      <div>
        ...
        >
          <center>
            <p className="p-0 m-0">MY MONSTERS&nbsp;</p>
            <span
              className="pt-0 mt-0"
              style={{ fontSize: '30px', fontWeight: 'bold', color: '#555555' }}
            >
              {!monsterCount || monsterCount == 0 ? '0' : monsterCount}
            </span>
          </center>
...
export default MonsterGetTopage
