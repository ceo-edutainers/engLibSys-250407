...
export default function WordListReadingBookAForTutor({
  ...
  const idiomListView = () => {
    ...
    var seriesName = 'Video'
    var url = DB_CONN_URL + '/idiom-voca-list-video/'
    var Url = url + seriesName + '&' + yID
    // alert(Url)
  ...
  function handleIdiomVocaClick(autoid) {
    ...
    var url = DB_CONN_URL + '/insert-memory-idiom-video-voca-from-student/'
  ...
  function wordListView() {
    ...
    // alert(homework_id)
    var url = DB_CONN_URL + '/memory-word-list-from-only-video-memoryword-list/'
    ...
    const fetchData3 = async () => {
      try {
        ...
        alert('wordListView Error difficult words3')
      }
    }
  ...
  function handleWordAddDirect(word) {
    if (searchWordMeaning == '' || word == '') {
      ...
      // console.log('insert ' + word + 'into my wordbook')
      var url =
        DB_CONN_URL + '/insert-direct-memory-idiom-video-voca-from-student/'
      // alert(url)
      //ここでwordをいろんな形で変換？
      // alert(word)
