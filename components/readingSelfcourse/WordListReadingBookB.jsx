...
export default function WordListReadingBook({
  ...
  function getScript() {
    ...
    if (seriesName == 'Reading Triumphs') {
      ...
      var Url =
        url + seriesName + '&' + readingLevel + '&' + bookNum + '&' + storyNum
    } else if (seriesName == 'Oxford Reading Tree') {
      // alert('here')
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // alert(response.data[0].script)
        // getWordEach(response.data.response[0].script)
        // alert('here' + response.data.response[0].story)
        if (response.data.response.length > 0) {
          getWordEach(response.data.response[0].story)
          // alert(response.data.response[0].story)
          console.log('getwordeach:', response.data.response[0].story)
        } else {
          getWordEach('')
          alert(
            '登録されてないストーリーです。知らない単語を辞書で調べた後、単語帳に書いて提出してください。'
          )
        }
      } catch (error) {
        alert(error)
          ...
          <div className="row">
            <div className="col-lg-8 col-md-12">
              ...
              >
                ...
                >
                  保存された単語は単語Quizに出ます。(9月上旬からサービス開始)
                  <br />
                  &nbsp;
                  ...
                  >
                    ...
                    <span style={{ fontSize: '14px' }}>
                      ...
                      {val2.ex1 && (
                        <>
                          <br />
                          <span className="pl-5" style={{ color: 'green' }}>
                            {val2.ex1}
                          </span>
                        </>
                      )}
