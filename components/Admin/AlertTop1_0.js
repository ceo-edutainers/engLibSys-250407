...
import ViewPaymentStudentList from '@/components/Admin/ViewPaymentStudentList'

import ViewStudentListOrderbyDate from '@/components/Admin/ViewStudentListOrderbyDate'
import ViewStudentListOrderbyName from '@/components/Admin/ViewStudentListOrderbyName'
...
const AlertTop2 = () => {
  ...
  const [ingMemberNameListView, setIngMemberNameListView] = useState(false)

  const [stopMemberDateListView, setStopMemberDateListView] = useState(false)
  const [stopMemberNameListView, setStopMemberNameListView] = useState(false)
  const [groupmemberListView, setGroupMemberListView] = useState(false)
  const [orderBy, setOrderBy] = useState('apply_date')
  ...
  return (
    <>
      ...
      >
        <h2 style={{ fontWeight: '600' }}>入金管理</h2>
      </div>
      <div className="courses-meta ml-3 mr-3">
        <div className="row">
          ...
          <div className="col-lg-10 col-md-12   mt-2 ">
            <span
              ...
              onClick={() => {
                ...
                setStopMemberNameListView(false)
                setStopMemberDateListView(false)
                setIngMemberNameListView(false)
              }}
            >
            ...
            <span
              ...
              style={{ cursor: 'pointer' }}
              onClick={() => {
                setIngMemberNameListView(!ingMemberNameListView)
                setGroupMemberListView(false)
                setStopMemberDateListView(false)
                setStopMemberNameListView(false)
                setIngMemberDateListView(false)
              }}
            >
              {ingMemberNameListView
                ? 'ing/name順 会員リストを隠す'
                : 'ing/name順 会員リストを見る'}
            </span>
            <span
              ...
              style={{ cursor: 'pointer' }}
              onClick={() => {
                setStopMemberDateListView(!stopMemberDateListView)
                setGroupMemberListView(false)
                setIngMemberNameListView(false)
                setIngMemberDateListView(false)
                setStopMemberNameListView(false)
            ...
            <span
              ...
              style={{ cursor: 'pointer' }}
              onClick={() => {
                setStopMemberNameListView(!stopMemberNameListView)
                setGroupMemberListView(false)
                setIngMemberNameListView(false)
                setIngMemberDateListView(false)
                setStopMemberDateListView(false)
          ...
          >
            <ViewStudentListOrderbyDate status="ing" />{' '}
          </div>
          <div
            style={{
              display: ingMemberNameListView ? 'block' : 'none',
            }}
          >
            <ViewStudentListOrderbyName status="ing" />{' '}
          </div>

          <div
            style={{
              display: stopMemberDateListView ? 'block' : 'none',
            }}
          >
            <ViewStudentListOrderbyDate status="stop" />
          </div>
          <div
            style={{
              display: stopMemberNameListView ? 'block' : 'none',
            }}
          >
            <ViewStudentListOrderbyName status="stop" />
          </div>
        </div>
      </div>
      <hr />
