...
const HurikaeAskT = () => {
  ...
  function getLessonDate8Times(weekday, start_time) {
    ...
    for (let i = 1; i < 9; i++) {
      ...
      //var dayofweek = myfun_getdayofweek(huriDateList2)
    ...
    //console.log('lessonDateEight-外:', lessonDateEight)
    console.log('arr-外:', arr)
    return true
  }

  function holidayCheck(checkDate) {
    //この日付が休みなのかを確認する
    axios.get(DB_CONN_URL + '/holiday-check/' + checkDate).then((response) => {
      //console.log('response.data.status', response.data)

      setThisHolidayCheck(response.data.status)
      console.log('thisHolidayCheck', thisHolidayCheck)
    })
    return thisHolidayCheck
  }
        ...
        <div className="row p-0 m-0">
          ...
          >
            <ul className="list-group">
              ...
              <li>
                レッスンを挟んで前後三日で振替スケジュールを設定できる->生徒がNoを送ったら、管理者と先生に通知がいく。管理者が他の先生を入れる。
              </li>
          ...
          <div className="col-sm-3 p-0  mr-2">
            ...
            <DatePicker
              ...
              minDate={new Date()}
              maxDate={new Date(moment().add(2, 'months'))} //最大3ヶ月後のスケジュール
              // filterDate={(date) =>
          ...
          <div className="col-sm-12 mt-3 p-0 text-center ">
            ...
            >
              リスト追加後、最後にこのボタンを押して担当先生に送信して下さい。
            </button>
          </div>
